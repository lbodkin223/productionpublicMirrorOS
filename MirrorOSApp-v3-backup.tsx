    ...
    try {
      ...
      if (response.ok) {
        setResult({
          ...
          success_timeline: data.chain_of_thought?.animation_sequence?.interactive_elements?.find((e: any) => e.type === 'success_timeline')
        });
      } else {
            ...
            <View style={styles.resultCard}>
              ...
              {result.evidence_summary && result.evidence_summary.length > 0 && (
                <View style={styles.evidenceSection}>
                  ...
                  {result.evidence_summary.map((evidence: any, idx: number) => (
              ...
              {result.key_success_factors && result.key_success_factors.length > 0 && (
                <View style={styles.successFactorsSection}>
                  ...
                  {result.key_success_factors.map((factor: any, idx: number) => (
              ...
              <View style={styles.chainOfThoughtSection}>
                ...
                {showChainOfThought && (
                  <View style={styles.chainContent}>
                    {result.chain_of_thought.reasoning_steps && result.chain_of_thought.reasoning_steps.map((step: any, index: number) => (
                      ...
                      <View style={styles.analysisSection}>
                        ...
                        {result.math_breakdown.positive_factors && result.math_breakdown.positive_factors.length > 0 && (
                          <View style={styles.factorSection}>
                            ...
                            {result.math_breakdown.positive_factors.map((factor: any, idx: number) => (
                        ...
                        {result.math_breakdown.negative_factors && result.math_breakdown.negative_factors.length > 0 && (
                          <View style={styles.factorSection}>
                            ...
                            {result.math_breakdown.negative_factors.map((factor: any, idx: number) => (
                              <Text key={idx} style={styles.factorText}>
                                • {factor.name}: {(factor.contribution * 100).toFixed(1)}%
                              </Text>
                            ))}
